...
class _MeetingPointScreenState extends State<MeetingPointScreen> {
  ...
  void initState() {
    ...
    if (args?.originLat != null) {
      context.read<MeetingBloc>().add(SelectMeetingPoint(
            args!.originLat!,
            args.originLng!,
            args.originAddress?.isNotEmpty == true
                ? args.originAddress!
  ...
  Future<void> _startSearch() async {
    ...
    try {
      ...
      final result = await _repository.createTrip(
        ...
        pickupLat: finalLat,
        pickupLng: finalLng,
        pickupAddress: (finalAddress?.isNotEmpty == true)
            ? finalAddress!
            : l10n.unspecified,
        destLat: args.destLat ?? 0.0,
        destLng: args.destLng ?? 0.0,
        destAddress: (args.destAddress?.isNotEmpty == true)
            ? args.destAddress!
            : l10n.unspecified,
        ...
        scheduledAt: args.scheduledAt, // Fix #16
      );
      ...
      if (waypoints != null && waypoints.isNotEmpty) {
        try {
          ...
          await Future.delayed(
              const Duration(milliseconds: 500)); // wait for trigger to finish
          var planData = await SupabaseService.client
              .from('trip_route_plans')
              .select('id')
              .eq('trip_id', result['id'])
              .maybeSingle();

          if (planData == null) {
            await routeRepo.createRoutePlanFromLegacy(result['id']);
            planData = await SupabaseService.client
                .from('trip_route_plans')
                .select('id')
                .eq('trip_id', result['id'])
                .maybeSingle();
                ...
                child: Padding(
                  ...
                  child: Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text(AppLocalizations.of(context)!.meetingPoint,
                          style: TextStyle(
            ...
            child: Center(
              child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(AppLocalizations.of(context)!.tapMapToSelect,
          ...
          Positioned(
            ...
            child: BlocBuilder<MeetingBloc, MeetingState>(
              builder: (context, state) =>
                  _buildBottomSheet(context, state, isDark, args),
            ),
          ),
        ],
      ),
  ...
  Widget _buildMap(
      bool isDark, LatLng origin, MeetingPointArgs? args, MeetingState state) {
  ...
  Widget _buildBottomSheet(BuildContext context, MeetingState state,
      bool isDark, MeetingPointArgs? args) {
    return Container(
                  ...
                  onTap: () {
                    ...
                    if (a?.originLat != null) {
                      context.read<MeetingBloc>().add(SelectMeetingPoint(
                            a!.originLat!,
                            a.originLng!,
                            a.originAddress?.isNotEmpty == true
                                ? a.originAddress!
                                : AppLocalizations.of(context)!.startingPoint,
                          ));
                      _mapController.future.then((ctrl) => ctrl.animateCamera(
                            CameraUpdate.newLatLngZoom(
                                LatLng(a.originLat!, a.originLng!), 15),
                          ));
                    }
                  },
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(AppLocalizations.of(context)!.resetToOrigin,
                          style: TextStyle(
